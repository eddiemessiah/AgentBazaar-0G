...
class Verifier extends base_1.ZGServingUserBrokerBase {
     ...
     */
    async verifyService(providerAddress, outputDir = '.', onLog) {
        const steps = [];
        const log = (type, message) => {
            const step = { type, message };
            steps.push(step);
            onLog?.(step);
        };
        try {
            ...
            for (const [reportType, report] of Object.entries(reports)) {
                ...
                if (reportSignerAddress) {
                    ...
                    log(addressMatch ? 'success' : 'error', `   Address Match: ${addressMatch ? '✅ MATCH' : '❌ MISMATCH'}`);
            ...
            const verificationSummary = {
                ...
                allVerificationsPassed: composeVerificationPassed && signerAllMatch,
            };
            log('step', '📋 Automated Verification Summary');
            log(verificationSummary.composeVerification ? 'success' : 'error', `   Docker Compose Verification: ${verificationSummary.composeVerification
                ? '✅ PASSED'
                : '❌ FAILED'}`);
            log(verificationSummary.signerAddressVerification ? 'success' : 'error', `   TEE Signer Address Verification: ${verificationSummary.signerAddressVerification
                ? '✅ PASSED'
     ...
     */
    async processDStackVerification(reports, log) {
        const allImages = [];
        ...
        for (const [reportType, report] of Object.entries(reports)) {
            ...
            try {
                ...
                log(composeResult.isValid ? 'success' : 'error', `     Status: ${composeResult.isValid ? '✅ VALID' : '❌ INVALID'}`);
